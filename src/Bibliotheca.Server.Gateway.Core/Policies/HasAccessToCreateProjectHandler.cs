...
{
    public class HasAccessToCreateProjectHandler : AuthorizationHandler<HasAccessToCreateProjectRequirement>
    {
        private readonly IUsersClient _usersClient;

        public HasAccessToCreateProjectHandler(IUsersClient usersClient)
        {
            _usersClient = usersClient;
        }

        protected override async Task HandleRequirementAsync(AuthorizationHandlerContext context, HasAccessToCreateProjectRequirement requirement)
        {
                ...
                || context.User.Identity.AuthenticationType == "UserToken")
            {
                ...
                if (user != null && (user.Role == RoleEnumDto.Administrator || user.Role == RoleEnumDto.Writer))
                {
                    context.Succeed(requirement);
                }
            }
